...
def coarse_grain(word, window: int, graph: bool = False) -> np.ndarray:
    if type(word) == str:
    ...
    if graph:
        th_cs_digits = np.zeros(len(coarse_grained))
        for i in range(len(coarse_grained) - 1):
            if coarse_grained[i + 1] > 0:
                th_cs_digits[i + 1] = th_cs_digits[i] + 1
            else:
                th_cs_digits[i + 1] = th_cs_digits[i] - 1
        plt.plot(th_cs_digits)
        plt.title(f"Dyck path coarse grained (grain l={window})")
        plt.show()
...
def main(n: int, prob: float, max_depth: int, new_leaves: int, g: bool = False) -> None:
    ...
    if g:
        ...
        plt.title(
            rf"Dyck word depths ($n$={n}, $p$={prob}, $d$={max_depth}, $k$={new_leaves})"
        )
    ...
    # TODO
    # Make sense of the coarse grained chains
    cs_digits = coarse_grain(digits_length, 21, True)
